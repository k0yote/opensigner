...
description: Explore six OpenSigner deployment scenarios from fully self-hosted to fully managed, with security trade-offs and custody implications for each configuration.
---
...
Due to the modular nature of the system, users can run or implement their own components.
Hybrid scenarios are possible where Openfort hosts some components while others are self-hosted.
...
- **Total Asset Ownership**: Since the hosting party controls **all** the Keys components, they can also decrypt cold shares if those belong to a project they created.
    The hosting party has access to both encryption shares and to the encrypted cold shares stored in the cold storage.
...
This is a common scenario: the implementations provided by Openfort are self-hosted,
...
In this scenario, the third party controls the cold and hot storages.
Combined with the brute-force risk mentioned in Scenario 3,
this can be enough to reconstruct the users' private keys.
...
2. **Automatic recovery with OTP**: there is an encryption key; split between the user and the cold storage,
...
:::important[OTP Recommendation for Automatic Recovery]
...
configuring automatic recovery. **Using OTP is strongly recommended to protect user accounts in this scenario.**
:::

Another important aspect to take into account when using automatic recovery is who owns what resources.
If the organization in charge of the cold storage starts a project within it, the organization can reconstruct the project-wide encryption key on their own.
Projects must be managed by someone who doesn't directly control the cold storage to avoid this scenario.

## Custody and key reconstruction

### Backend access to key shares

**In automatic recovery mode without OTP, the answer depends on the hosting configuration.**

- **Scenario 6 (Fully Hosted by a Host such as Openfort): No.**
  The Host (such as Openfort) holds the *Shield Encryption Part*. You (Developer) hold the *Developer Encryption Part*.
...
**In password/passkey recovery or automatic recovery with OTP: No.**
...
### Protection mechanisms for non-custodial deployments
...
### Open-source custody model

**The open-source configuration is custodial by default when you host everything yourself.** In a fully self-hosted setup (Scenario 1):
...
The open-source OpenSigner configuration, when using automatic recovery without OTP, meets this technical definition of custody because the platform can access both shares needed for key reconstruction.
...
<JsonLd data={techArticle({ headline: 'Deployment scenarios', description: 'Explore six OpenSigner deployment scenarios from fully self-hosted to fully managed, with security trade-offs and custody implications for each configuration.', path: '/security/deployment-scenarios' })} />
